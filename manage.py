...
'''
    ...
    ----------
    Простой поиск
    Реализуем мы его через простой GET запрос с определёнными параметрами
    Впринципе, мы уже умеем работать с экземплярами моделей и вытаскивать
    их по определённому атрибуту.

    Сами параметры хранятся в словаре GET у объекта запроса.
    Мы можем получить значение из этого словаря по ключу, который мы можем
    задать input-элементу атрибутом name

    После присваивания значения name, мы можем обращаться к именованному
    элементу, что в коде и делаем

    Если это будет ключ в атрибуте запроса, значит, должен быть и сам
    запрос? Да, его будет делать сама форма, функцию обработчкик
    мы укажем в атрибутах самой формы, в action="{% имя_функции %}"
    В самом коде я написал {% url 'posts_list_url' %}
    это значит, что будет вызвана функция, связанная с именем указанного узла,
    которое я описал в urls.py текущего приложения

    В данном случае запрос будет обрабатывать post_list
    Там логика поиска поста и будет реализована

    Ещё важный момент: в джанго можно задать несколько условий
    выборки в фильтре, причём каждый со своими ключами типа
    __iexact или __icontains. Перечисляя их через запятую они
    как-будто объединяются через and, т.е. совпадение всех
    одновременно для одной записи обязательно.

    А чтобы искать через ИЛИ или задать более сложный фильтр,
    достаточно импортировать класс: 
    from django.db.models import Q
    Чтобы условия работали выше описанным способом, сделаем так:
    posts = Post.objects.filter(Q(title__icontains=search_query) | Q(body__icontains=search_query))
    И вроде всё, задача выполнена
'''